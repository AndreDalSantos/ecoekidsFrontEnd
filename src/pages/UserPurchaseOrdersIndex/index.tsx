...
const UserPurchseOrders: React.FC = () => {
  ...
  useEffect(() => {
    if (!user) {
      ...
      api.get(`/orders/orders_of_customer/${user.id}`).then(response => {
  ...
  return (
    <div>
      ...
      <OrdersContainer>
        <div className="title">
          <h3>Listagem dos meus pedidos de compra:</h3>
        </div>
        {orders?.map(item => {
          return (
            <OrderCard key={item.id}>
              {/* <button type="button" onClick={handleShowOrder(item.id)}> */}
              <Link to={`private_user_purchse_order/${item.id}`}>
                <div>
                  <p>
                    <strong>id do pedido:</strong> {item.id}
                  </p>
                  <p>
                    <strong>Vendedor:</strong>{' '}
                    {item.seller ? item.seller.name : 'Usuário removido'}
                  </p>
